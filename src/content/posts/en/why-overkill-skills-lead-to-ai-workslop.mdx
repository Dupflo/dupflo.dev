...
description: I ran RPI and its twenty-three agent skills on a six-file change. It produced 3,418 lines of planning and estimated seven to ten person-days. I shipped it in thirty minutes with one prompt, reading the files and correcting them by hand.
date: 2026-08-11
lang: en
...
draft: false
---
I am always looking to optimize my process. And to be honest, that behaviour is exactly what leads me to produce AI workslop today.
...
![Comic strip: ten people crowded around a conference table arguing over the color of a single UI button on the projector screen, while one exhausted developer thinks "It's a button."](../../../assets/posts/overkill-skills/button-color-meeting.png)
...
| File | Lines | Content |
| --- | --- | --- |
| `pm.md` | 730 | 12 user stories, 19 acceptance criteria, 8 arbitrated PO questions |
| `ux.md` | 813 | 3 flows, 14 states, 40 i18n keys |
| `eng.md` | 1385 | API contract, types, real code, 12 risks |
| `PLAN.md` | 490 | 5 phases, 44 tasks |
...
I took a step back and said to myself: this is the ten-people-meeting for a button color. Not the same room, the same failure mode. More roles and more artifacts than the decision required.

The HBR definition covers work handed to a colleague. Mine was handed to me, by me, which is why I want to widen it by one notch: framing tools like RPI or gstack produce the same thing when they meet a problem too small for them. Analysis nobody needed, planning nobody will read, and a cleanup cost paid by the only person in the room. The site you are reading this on was built with Claude and no skills at all, no RPI, no gstack, nothing. It did not need them, and adding them would have produced exactly this.

Reading the AI report was more painful than writing the code myself.
...
But read the same threads to the end and the most important tip is the same one: make sure Claude has a way to verify its work. So trust does not exclude control. It just means the control is verification, not a fifty-page plan written before a single line of code.

AI processes are non-deterministic, like AI itself. There is no single way to work that fits every project, and that comes from the Claude Code team too, and Boris describes [his own setup](https://github.com/shanraisshan/claude-code-best-practice/blob/main/tips/claude-boris-13-tips-03-jan-26.md) as "surprisingly vanilla." The person who built the tool barely customizes it. Meanwhile I was stacking RPI on top of skills on top of plugins for a six-file change.
...
The best example is LinkedIn. Open your feed, look at the last five posts, and tell me how many were written by hand and carry real information from someone who has mastered a skill. AI gives people the illusion of productivity: AI-written posts, filed under "sharing expertise."
